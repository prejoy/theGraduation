...
u8 mem2base[MEM2_MAX_SIZE]__attribute__((section(".mymalloc")))__attribute__((aligned(32)));	//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)
u8 mem3base[MEM3_MAX_SIZE] __attribute__((aligned(32)));					//内部CCM内存池
//内存管理表
u32 mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
u32 mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".mymalloc")));//__attribute__((at(0XC01F4000+MEM2_MAX_SIZE)));	//外部SRAM内存池MAP
