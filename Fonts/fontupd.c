...
//pos:当前文件指针位置
u32
fupd_prog (u16 x, u16 y, u8 size, u32 fsize, u32 pos)
{
  float prog;
  u8 t = 0XFF;
...
//返回值:0,成功;其他,失败.
u8
updata_fontx (u16 x, u16 y, u8 size, u8 *fxpath, u8 fx)
{
  u32 flashaddr = 0;
  FIL * fftemp;
  u8 *tempbuf;
...
//		 其他,错误代码.	  
u8
update_font (u16 x, u16 y, u8 size, u8* src)
{
  u8 *pname;
  u32 *buf;
  u8 res = 0;
...
//    return 1;
  ftinfo.fontok = 0xaa;
  ftinfo.ugbkaddr = 0x01900029;
  ftinfo.ugbksize =0x0002A908;
  ftinfo.f12addr=0x0192A931;
  ftinfo.gbk12size= 0x0008C460;
  ftinfo.f16addr =0x019B6D91;
  ftinfo.gbk16size=0x000BB080;
  ftinfo.f24addr=0x01A71E11;
  ftinfo.gbk24size=0x001A4D20;
  ftinfo.f32addr=0x01C16B31;
  ftinfo.gbk32size=0x002EC200;
