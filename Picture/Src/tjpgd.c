...
{  
	...
	if(res==0)
	{
		...
		if(res==FR_OK)//打开文件成功
		{ 
			res=jd_prepare((JDEC*)jpeg_dev,(void*)jpeg_in_func,(void*)jpg_buffer,(UINT)JPEG_WBUF_SIZE,(void*)f_jpeg);//执行解码的准备工作，调用TjpgDec模块的jd_prepare函数
			outfun=(void*)jpeg_out_func_point;//默认采用画点的方式显示
			...
			{ 	
				...
				{ 
					...
					{	
						...
						else outfun=(void*)jpeg_out_func_fill;	//在显示尺寸以内,可以采用填充的方式显示
						break; 							
					} 
				} 
