...
{				
	...
	if((x+width-1)>lcdltdc.width)return PIC_WINDOW_ERR;		//区域错误
	if((y+height-1)>lcdltdc.height)return PIC_WINDOW_ERR;	//区域错误
 	...
 	if(res==FR_OK)//创建成功
	{
		...
		for(ty=y+height-1;hbmp.bmiHeader.biHeight;ty--)
		{
			...
			{
				if(pixcnt<hbmp.bmiHeader.biWidth)databuf[pixcnt]=LTDC_Read_Point(tx,ty);//读取坐标点的值
