...
{
	if((len==0)||(x0>lcdltdc.width)||(y0>lcdltdc.height))return;
...
{  
	u16 i,j;
	if(lcdltdc.pwidth!=0&&lcdltdc.dir==0)//如果是RGB屏,且竖屏,则填充函数不可直接用
	{
		for(i=0;i<height;i++)
		{
			for(j=0;j<width;j++)
	...
	}else LTDC_Color_Fill(x,y,x+width-1,y+height-1,color);//其他情况,直接填充
}
//////////////////////////////////////////////////////////////////////////
...
void piclib_init(void)
{
	...
	picinfo.lcdwidth=lcdltdc.width;	//得到LCD的宽度像素
	picinfo.lcdheight=lcdltdc.height;//得到LCD的高度像素
...
{	
	...
	{
		picinfo.S_Height=lcdltdc.height;
		picinfo.S_Width=lcdltdc.width;
		return FALSE;   
	}
